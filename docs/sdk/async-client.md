...
`LunarisClient` 是 Lunaris 的异步客户端，基于 WebSocket 实现实时通信。
...
```python
async with LunarisClient("ws://localhost:8000", "token") as client:
    # 自动连接和关闭
    ...
```
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `wasm_module` | `bytes \| str` | - | WASM 模块字节码或源代码 |
| `args` | `List[Any]` | `None` | 任务参数列表 |
| `entry` | `str` | `"wmain"` | 入口函数名 |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `wasm_module` | `bytes \| str` | WASM 模块 |
| `args_list` | `List[List[Any]]` | 多组参数列表 |
| `timeout` | `float` | 超时时间（秒） |

**返回值**：任务结果列表，顺序与 `args_list` 一致
...
```python
async def unsubscribe_tasks(self, task_ids: List[str]) -> None: ...
```
...
- 查看 [同步客户端](sync-client.md) 了解同步 API
